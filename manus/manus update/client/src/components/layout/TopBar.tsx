    ...
    <header className="topbar">
      ...
      <nav className={`topbar-nav ${menuOpen ? "topbar-nav-open" : ""}`} aria-label="Primary navigation">
        {navItems.map((item, index) => (
          <a key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className={item.href === "/exchange" ? "nav-category nav-category-exchange" : item.href === "/markets" ? "nav-category nav-category-market" : ""}>
