...
import { walletBalance } from "@/lib/flint-data";
    ...
    <div className="flint-app category-page category-page-markets">
      ...
      <main>
        <header className="category-hero">
          ...
          <aside className="category-summary category-summary-market"><WalletCards size={16} /><span className="metric-label">AVAILABLE BALANCE</span><strong className="mono">{walletBalance.available}</strong><p>Open positions and market capital remain separate from worker escrow.</p><div className="category-summary-foot mono"><span>24 OPEN BOOKS</span><span>600 USDC STAKED</span></div></aside>
        </header>
        <div id="book"><PredictionMarket /></div>
        <div className="market-support-grid" id="positions"><SBTPanel /><ProtocolStrip /></div>
      </main>
      <footer className="site-footer"><div className="footer-brand"><span className="brand-mark brand-mark-small" aria-hidden="true"><span className="brand-mark-cut" /></span><span className="brand-name">FLINT</span><span className="mono">MARKETS / V1</span></div><div className="footer-links"><a href="/exchange"><Shield size={13} /> GIG EXCHANGE</a><a href="#book"><WalletCards size={13} /> CURRENT BOOK</a><a href="#positions"><Github size={13} /> POSITION LEDGER</a></div><a className="back-top mono" href="#top">BACK TO TOP <ChevronUp size={13} /></a></footer>
    </div>
  );
}
